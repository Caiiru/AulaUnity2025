...
public class Douglas_PlayerControl : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        gameManager = GameObject.FindFirstObjectByType<Douglas_GameManager>(); // Procura o primeiro obj com o script GameManager.
    }
    ...
    void InputKeys()
    {
        if (Keyboard.current.wKey.isPressed) // Ve se esta precionando a tecla W e entao adiciona um valor positivo a aceleracao.
            aceleration = 1;
        ...
        if (Keyboard.current.dKey.isPressed)    // Ve qual lado o player quer girar e adiciona um valor positivo ou negativo a velocidade de rotacao.
    ...
    void Jump()
    {
        if (Keyboard.current.spaceKey.isPressed && isGrounded) // Se estiver no chao e precionar espaco adiciona um impulso pra cima.
